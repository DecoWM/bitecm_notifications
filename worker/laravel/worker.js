  ...
  if (worker().checkIfValid(job)) {
    job.fire(function(status) {
      switch(status) {
        ...
        break;
        case Job.FOR_LATER:
          Log.info('Job #%s not ready', job.id);
          worker().retryLater(job);
        break;
        case Job.CORRUPTED:
          Log.info('Job #%s corrupted', job.id);
          worker().terminateJob(job);
        break;
        case Job.FAILED:
          Log.info('Job #%s failed', job.id);
          //do nothing
        break;
      }
    });
...
Worker.prototype.terminateJob = function(job) {
  queue.delete(job, function (err) {
    if (!err) {
      Log.info('Job #%s terminated', job.id);
    }
  });
};
...
Worker.prototype.retryLater = function(job) {
  queue.reserveUnlock(job, function (err) {
    if (!err) {
      Log.info('Job #%s pushed for later retry', job.id);
    }
  });
};
